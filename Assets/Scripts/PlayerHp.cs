...
public class PlayerHp : MonoBehaviour
{
    ...
    {
        // Check if the colliding object's position touches the specified X-axis value (-12)
        if (other.transform.position.x <= -12f)
        {
            TakeDamage(10); // Reduce health by 10 when the object touches the specified X-axis
        }
    }
}
